...
class YamlToNsisConverter(BaseConverter):
    ...
    def _generate_signing_section(self) -> List[str]:
        ...
        lines = [
            ...
            f'; Verify signature after build: {self.config.signing.verify_signature}',
            f'; Checksum: {self.config.signing.checksum_type} {self.config.signing.checksum_value}',
    ...
    def _generate_update_section(self) -> List[str]:
        ...
        lines = [
            ...
            f'!define DOWNLOAD_URL "{self.config.update.download_url}"',
            f'!define CHECK_ON_STARTUP "{str(self.config.update.check_on_startup).lower()}"',
            f'!define BACKUP_ON_UPGRADE "{str(self.config.update.backup_on_upgrade).lower()}"',
            f'!define REPAIR_ENABLED "{str(self.config.update.repair_enabled).lower()}"',
            "",
            ...
            f'  WriteRegStr {self.config.update.registry_hive} "{self.config.update.registry_key}" "UpdateURL" "${{UPDATE_URL}}"',
            f'  WriteRegStr {self.config.update.registry_hive} "{self.config.update.registry_key}" "DownloadURL" "${{DOWNLOAD_URL}}"',
            f'  WriteRegStr {self.config.update.registry_hive} "{self.config.update.registry_key}" "CheckOnStartup" "${{CHECK_ON_STARTUP}}"',
            f'  WriteRegStr {self.config.update.registry_hive} "{self.config.update.registry_key}" "BackupOnUpgrade" "${{BACKUP_ON_UPGRADE}}"',
            f'  WriteRegStr {self.config.update.registry_hive} "{self.config.update.registry_key}" "RepairEnabled" "${{REPAIR_ENABLED}}"',
            "SectionEnd",
            "",
        ]
        return lines
